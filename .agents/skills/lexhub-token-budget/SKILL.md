...
o'zgartirishdan** keladi. Isbotni tashlab ketish token tejamaydi — u AGENTS.md
ning CLAIM ≠ EVIDENCE qoidasini buzadi va soxta success ishlab chiqaradi.

## 1. Shu loyihada O'LCHANGAN sarf profili

Manba: `~/.Codex/projects/D--projects-LexHub/*.jsonl` — 4 fayl, 13 327 yozuv,
...
hook'lari (`~/.Codex/settings.json` → `.orca/agent-hooks`) conhost terminal
...
`path:line → fakt` ko'rinishida. Ledger — `.Codex/notes/<task>.md` yoki
...
1. AGENTS.md ning 5 VERIFIED sharti. Real environment, real runtime, kutilgan
...
```bash
cd "$HOME/.Codex/projects/D--projects-LexHub" && PYTHONIOENCODING=utf-8 python -c "
